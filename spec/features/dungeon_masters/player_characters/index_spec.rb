...
RSpec.describe 'dungeon master and player_character index' do
    ...
    it 'can show each child with their attributes associated with the parent' do
        lucius = DungeonMaster.create!(name: 'Ozy', number_of_players: 3, dm_active: 'true', level_range: 'high')
        ...
        expect(page).to_not have_content(adventurer4.name)
        ...
        expect(page).to have_content(advenurer1.updated_at)
    end
end